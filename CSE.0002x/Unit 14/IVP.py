...
def step_RK4(thisIVP,dt,un,tn):
    """
    Takes a single timestep of the 4th order Runge-Kutta method (RK4) 
    to (approximately) integrate the state from u(tn) to u(tn+dt)
    ...
    c = dt*thisIVP.evalf(un + 0.5*b, tn + 0.5*dt)

    # compute the d vector
    d = dt*thisIVP.evalf(un +     c, tn +     dt)
    
    # compute the next step
    un1 = un + (a+2*b+2*c+d)/6.
...
def solve(thisIVP, dt, method):
    ...
    n = 0
    while (n<N-1 and not thisIVP.stopcheck(u[n,:])):
        u[n+1,:]= method(thisIVP,dt,u[n,:],t[n])
        n += 1 
